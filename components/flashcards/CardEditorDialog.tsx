...
// import { saveCard, uploadMedia } from "@/lib/storage"; // Adicione uploadMedia
...
import { Checkbox } from "@/components/flashcards/ui/checkbox";
import { uploadMedia } from "@/lib/storage"; // Importação adicionada
...
export function CardEditorDialog({
  ...
  const [isUploading, setIsUploading] = useState(false); // Novo estado para bloquear salvamento durante upload
  ...
  // --- Handlers de Upload Refatorados ---
  ...
  const handleBackImageUpload = async (file: File) => {
    setIsUploading(true);
    toast.info("Uploading back image...");
    try {
      const url = await uploadMedia(file);
      if (url) {
        setBackImageUrl(url);
  ...
  const handleBackAudioUpload = async (file: File) => {
    setIsUploading(true);
    toast.info("Uploading back audio...");
    try {
      const url = await uploadMedia(file);
      if (url) {
        setBackAudioUrl(url);
        toast.success("Back audio uploaded successfully");
      } else {
        toast.error("Failed to upload back audio");
      }
    } catch (error) {
      console.error(error);
      toast.error("Error uploading back audio");
    } finally {
      setIsUploading(false);
    }
  };

  // --- Fim dos Handlers de Upload ---
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={handleSubmit} disabled={isUploading}>
            {isUploading ? "Uploading..." : "Save Card"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
